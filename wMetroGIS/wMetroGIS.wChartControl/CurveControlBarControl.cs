...
{
	public class CurveControlBarControl : BaseChartControl
	{
			...
			if (this.m_ShowBar || this.m_ShowLine)
			{
				if (this.m_ChartData != null && this.m_ChartData.Length != 0)
				{
					...
					if (this.ChartGraph.ClientRectangle.Width != 0 && this.ChartGraph.ClientRectangle.Height != 0)
					{
						...
						if (this.m_ShowLine)
						{
							this.m_LineItem = myPane.AddCurve("数据曲线", x, y, this.CurveColor, SymbolType.HDash);
						...
						if (this.m_ShowBar)
						{
							this.m_BarItem = myPane.AddBar("数据", x, y, System.Drawing.Color.SteelBlue);
							this.m_BarItem.Bar.Fill = new Fill(this.BarColor1, this.BarColor2, this.BarColor1, this.BarColorAngle);
						}
